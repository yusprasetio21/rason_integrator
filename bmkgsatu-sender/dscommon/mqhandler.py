...
class MqHandler():
    ...
    def consume_create_channel(self):
        try:
            if not self.is_consume_connected():
                return None
    ...
    def start_consuming(self, queue, callback, auto_ack=True):
        ...
        while True:
            try:
                ...
                # This will cause the program to finish, disable for a moment
                # break  # Exit loop on an unexpected error
            
    def close(self):
        LOG.info(json.dumps({ "event": "Try to close MQ" }))
        try:
            if self.publish_connection:
                self.publish_connection.close()
        except Exception as e:
            LOG.warning(json.dumps({"event": "Error closing publish connection", "error": str(e)}))

        try:
            if self.consume_connection:
                self.consume_connection.close()
        except Exception as e:
            LOG.warning(json.dumps({"event": "Error closing consume connection", "error": str(e)}))
        LOG.info(json.dumps({ "event": "MQ closed" }))
