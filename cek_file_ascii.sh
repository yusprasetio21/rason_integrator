...
# === 1. Path dan Metadata ===
BINARY_DIR="/home/bmkgsatu/rason_integrator/rasonfile-bucket/sent/ascii"
OUTPUT_DIR="/home/bmkgsatu/rason_integrator/checkfile/"
TODAY=$(date +%Y-%m-%d)
TODAY_TS=$(date +"%Y%m%d")
OUTPUT_CSV="$OUTPUT_DIR/ascii_check_output_${TODAY_TS}.csv"

# Metadata station embedded langsung dalam skrip (wmo_id|station_name)
...
if [[ ! -d "$BINARY_DIR" ]]; then
    echo "Folder tidak ditemukan: $BINARY_DIR"
    exit 1
fi
...
for FILE_PATH in "${FILES[@]}"; do
    ...
    # Format: Dxxxxx_YYYYMMDDhh.DAT
if [[ "$FILE_NAME" =~ ^D([0-9]{5})_([0-9]{10})\.X$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="${BASH_REMATCH[2]:0:4}-${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:6:2} ${BASH_REMATCH[2]:8:2}:00"

# Format: [PT]20xxxxxXYYYYMMDDhhmm.X
elif [[ "$FILE_NAME" =~ ^[PT]20([0-9]{5})[A-Z0-9]([0-9]{12})\.X$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="${BASH_REMATCH[2]:0:4}-${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:6:2} ${BASH_REMATCH[2]:8:2}:${BASH_REMATCH[2]:10:2}"

# Format: Dxxxxx_YYYYMMDDhh.DAT
elif [[ "$FILE_NAME" =~ ^D([0-9]{5})_([0-9]{10})\.DAT$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="${BASH_REMATCH[2]:0:4}-${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:6:2} ${BASH_REMATCH[2]:8:2}:00"

# Format: [PT]20xxxxxXYYYYMMDDhhmm.DAT
elif [[ "$FILE_NAME" =~ ^[PT]20([0-9]{5})[A-Z]([0-9]{12})\.DAT$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="${BASH_REMATCH[2]:0:4}-${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:6:2} ${BASH_REMATCH[2]:8:2}:${BASH_REMATCH[2]:10:2}"

# Format: [PT]20xxxxxXddMMyyhhmm.DAT (Versi Lama)
elif [[ "$FILE_NAME" =~ ^[PT]20([0-9]{5})[A-Z]([0-9]{10})\.DAT$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="20${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:2:2}-${BASH_REMATCH[2]:0:2} ${BASH_REMATCH[2]:6:2}:${BASH_REMATCH[2]:8:2}"

# Format: PA96805_2025041400GTS.DAT atau TA96805_2025041400GTS.DAT
elif [[ "$FILE_NAME" =~ ^[PT][A-Z]([0-9]{5})_([0-9]{10})GTS\.DAT$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="${BASH_REMATCH[2]:0:4}-${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:6:2} ${BASH_REMATCH[2]:8:2}:00"

# Format: TA96805_2025041400.DAT
elif [[ "$FILE_NAME" =~ ^[PT][A-Z]([0-9]{5})_([0-9]{10})\.DAT$ ]]; then
    WMO_ID="${BASH_REMATCH[1]}"
    TANGGAL_DATA="${BASH_REMATCH[2]:0:4}-${BASH_REMATCH[2]:4:2}-${BASH_REMATCH[2]:6:2} ${BASH_REMATCH[2]:8:2}:00"
...
if command -v soffice >/dev/null 2>&1; then
    soffice --headless --convert-to csv "$OUTPUT_CSV"
else
    echo "OpenOffice/LibreOffice tidak ditemukan, konversi file CSV ke format Excel gagal"
fi
